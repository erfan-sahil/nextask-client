...
export const workflowApi = {
  ...
  async acceptWorkspaceInvitation(token: string) {
    return unwrap(
      await apiClient.post("/workspaces/invitations/accept", { token }),
    );
  },
  async previewProjectInvitation(token: string) {
    return unwrap(
      await apiClient.get("/projects/invitations/preview", {
        params: { token },
      }),
    );
  },
  async acceptProjectInvitation(token: string) {
    return unwrap(
      await apiClient.post("/projects/invitations/accept", { token }),
    );
  },
  async listWorkspaces(params: ListParams = {}) {
    return unwrap(
      await apiClient.get<
        ApiSuccessResponse<ListResult<WorkspaceDoc, "workspaces">>
      >("/workspaces", {
        params,
      }),
    );
  ...
  async getCalendarEvents({
    ...
    startDate,
    endDate,
  }: WorkspaceRef & { startDate: string; endDate: string }) {
    return unwrap(
      await apiClient.get<ApiSuccessResponse<{ events: CalendarEventDoc[] }>>(
  ...
  async createMeeting({
    ...
    return unwrap(
      await apiClient.post<ApiSuccessResponse<{ meeting: MeetingDoc }>>(
        `/workspaces/${workspaceId}/calendar/meetings`,
        input,
      ),
    ).meeting;
  },
  ...
  async listProjects({ workspaceId, ...params }: WorkspaceRef & ListParams) {
    return unwrap(
      await apiClient.get<
        ApiSuccessResponse<ListResult<ProjectDoc, "projects">>
  ...
  async updateProject({
    ...
    Partial<
      Pick<
        ProjectDoc,
        "name" | "description" | "icon" | "status" | "startDate" | "endDate"
      >
    >) {
    return unwrap(
  ...
  async createBoard({
    ...
    projectId,
    ...input
  }: ProjectRef & { name: string; description?: string }) {
    return unwrap(
  ...
  async updateBoard({
    ...
    boardId,
    ...input
  }: BoardRef & Partial<Pick<BoardDoc, "name" | "description">>) {
    return unwrap(
  ...
  async updateColumn({
    ...
    columnId,
    ...input
  }: BoardRef & { columnId: string } & Partial<
      Pick<ColumnDoc, "name" | "position" | "color" | "isCompletedColumn">
    >) {
    return unwrap(
  ...
  async listWorkspaceMembers({
    workspaceId,
    ...params
  }: WorkspaceRef & ListParams) {
    return unwrap(
      await apiClient.get<ApiSuccessResponse<ListResult<MemberDoc, "members">>>(
  ...
  async inviteWorkspaceMember({
    ...
    email,
    role,
  }: WorkspaceRef & { email: string; role?: MemberRole }) {
    return unwrap(
  ...
  async updateWorkspaceMember({
    ...
    memberId,
    role,
  }: WorkspaceRef & { memberId: string; role: MemberRole }) {
    return unwrap(
  ...
  async inviteProjectMember({
    ...
    email,
    role,
  }: ProjectRef & { email: string; role?: MemberRole }) {
    return unwrap(
  ...
  async updateProjectMember({
    ...
    memberId,
    role,
  }: ProjectRef & { memberId: string; role: MemberRole }) {
    return unwrap(
  ...
  async listComments({
    ...
    return unwrap(
      await apiClient.get<
        ApiSuccessResponse<ListResult<TaskCommentDoc, "comments">>
      >(
  ...
  async createComment({
    ...
    taskId,
    content,
  }: TaskRef & { content: string }) {
    return unwrap(
      await apiClient.post<ApiSuccessResponse<{ comment: TaskCommentDoc }>>(
        `/workspaces/${workspaceId}/projects/${projectId}/boards/${boardId}/tasks/${taskId}/comments`,
        { content },
      ),
    ).comment;
  ...
  async updateComment({
    ...
    commentId,
    content,
  }: TaskRef & { commentId: string; content: string }) {
    return unwrap(
      await apiClient.patch<ApiSuccessResponse<{ comment: TaskCommentDoc }>>(
