    ...
    <main className="mx-auto flex min-h-screen max-w-lg items-center px-4">
      <section className="w-full rounded-2xl border border-border bg-card p-6 shadow-sm">
          ...
          <p className="mt-4 text-sm text-muted-foreground">
            ...
            <strong className="text-foreground">
              {String(
                (
                  preview.data as {
                    invitation?: { project?: { name?: string } };
                  }
                ).invitation?.project?.name ?? "a project",
              )}
            </strong>
            . Accept with the invited email to get project-only access.
          </p>
        )}
        <div className="mt-6 flex gap-3">
          ...
          <Link
            href={`${authRoutes.login}?callbackUrl=${encodeURIComponent(`/project-invitations/accept?token=${token}`)}`}
